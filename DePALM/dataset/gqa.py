...
class GQAFineTuneDataset(Dataset):
    def __init__(
        ...
        vg_img_dir = vg_dir.joinpath("images/")  # VG_100K
        ...
        # self.imgid_to_vgdir = json.load(open(gqa_dir.joinpath(f'imgid_to_vgdir.json'), 'r'))
    ...
    def __getitem__(self, idx):
        ...
        path = self.featname_to_h5[featname]

        if "others" in featname:
            if img_id in self.imgid_to_vgdir:
                vg_subdir = self.imgid_to_vgdir[img_id][0]
                path = path.joinpath(vg_subdir)
