...
class TextVQAFineTuneDataset(Dataset):
    def __init__(
        ...
        self.featname_to_h5 = {
            "train": dataset_dir.joinpath("train_images"),
            "val": dataset_dir.joinpath("train_images"),
            "test": dataset_dir.joinpath("test_images"),
        }
    ...
    def __getitem__(self, idx):
        ...
        path = path.joinpath(f"{img_id}.jpg")
...
def get_loader(
    ...
    **kwargs,
    ...
    _dset = TextVQADataset(split, verbose, data_dir=data_dir)
    ...
    loader.evaluator = TextVQAEvaluator(_dset)
    loader.task = "vqa"
...
class TextVQADataset:
    ...
    def __init__(self, splits: str, verbose=True, data_dir="/data/mshukor/data"):
        ...
        textvqa_dir = dataset_dir

        # Loading datasets to data
        self.data = []
        for split in self.splits:
            data = json.load(open(textvqa_dir.joinpath("%s.json" % split)))
            self.data.extend(data)
        if verbose:
            print("Load %d data from split(s) %s." % (len(self.data), self.name))

        # List to dict (for evaluation and others)
        self.id2datum = {int(datum["question_id"]): datum for datum in self.data}
...
class TextVQAEvaluator:
    def __init__(self, dataset: TextVQADataset):
