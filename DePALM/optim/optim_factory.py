...
def create_optimizer(args, model, filter_bias_and_bn=True, config=None):
    ...
    lora_lr = config.get("lora_lr", args.lr)
    ...
    if customized_lr:
        ...
        for kv in model.named_parameters():
            ...
            elif "cross_modal_module" in kv[0].split(".")[0]:
                params["cross_modal_module"].append(kv[1])
        ...
        parameters.append(
            {
                "params": params["cross_modal_module"],
                "lr": connector_lr,
                "weight_decay": weight_decay,
            }
        )
        print("cross_modal_module", len(params["cross_modal_module"]))

        # model_vision_params = [kv[1] for kv in model.named_parameters() if 'model_vision' in kv[0].split('.')[0]]
        parameters.append(
            {
                "params": params["model_vision"],
                "lr": vis_lr,
                "weight_decay": weight_decay,
            }
        )
        ...
        parameters.append(
            {"params": params["prompt"], "lr": prompt_lr, "weight_decay": weight_decay}
        )
        ...
        adapter_params = [
            kv[1] for kv in model.named_parameters() if "adapter" in kv[0]
        ]
        ...
        other_params = [
            kv[1]
            for kv in model.named_parameters()
            if "adapter" not in kv[0] and "prompt" not in kv[0]
        ]
        parameters = [
            {"params": other_params},
    ...
    elif config.get("lora_lr", False) and config.get("prompt_lr", False):
        lora_params = [kv[1] for kv in model.named_parameters() if "lora" in kv[0]]
        promt_params = [kv[1] for kv in model.named_parameters() if "prompt" in kv[0]]

        other_params = [
            kv[1]
            for kv in model.named_parameters()
            if "lora" not in kv[0] and "prompt" not in kv[0]
        ]
        parameters = [
            {"params": other_params},
            {"params": promt_params, "lr": prompt_lr, "weight_decay": weight_decay},
        ...
        other_params = [
            kv[1] for kv in model.named_parameters() if "adapter" not in kv[0]
        ]
        parameters = [
            {"params": other_params},
            {"params": adapter_params, "lr": adapter_lr, "weight_decay": weight_decay},
        ...
        promt_params = [kv[1] for kv in model.named_parameters() if "prompt" in kv[0]]
        other_params = [
            kv[1] for kv in model.named_parameters() if "prompt" not in kv[0]
        ]
        parameters = [
            {"params": other_params},
            {"params": promt_params, "lr": prompt_lr, "weight_decay": weight_decay},
